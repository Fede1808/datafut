      ...
      {filas.map((f) => {
        ...
        return (
          <div
            key={f.clave}
            role="row"
            className="flex items-center gap-1.5 border-b border-[#201e1b] py-2"
          >
            ...
            <span
              role="cell"
              className="num w-12 shrink-0 text-right text-[12px] text-[#a09a90]"
            >
            ...
            >
              ...
              <span
                className={`num w-[38px] text-right text-[11.5px] ${
                  Math.abs(d) >= 5 ? "text-[#ffbe3d]" : "text-[#6d6862]"
                }`}
              >
                {d > 0 ? "+" : d < 0 ? "−" : "±"}
...
function BarraDiferencia({ valor }: { valor: number }) {
  ...
  return (
    <svg width={ancho} height={alto} viewBox={`0 0 ${ancho} ${alto}`} aria-hidden>
      <rect x={0} y={alto / 2 - 0.5} width={ancho} height={1} fill="#201e1b" />
      <rect
      ...
      />
      <rect x={medio - 0.5} y={0} width={1} height={alto} fill="#423e38" />
    </svg>
  );
}
