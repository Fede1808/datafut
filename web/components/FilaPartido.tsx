...
export function FilaPartido({ p }: { p: Partido }) {
  ...
  return (
    <Link
      ...
      className="fila block border-b border-borde px-3 py-2.5 last:border-b-0"
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="num w-5 shrink-0 text-[9.5px] text-tinta4">
          {l ? `${l.puesto}°` : ""}
        </span>
        ...
        <span className="num shrink-0 text-[9px] uppercase tracking-[0.1em] text-tinta4">
          vs
        </span>
        ...
        <span className="num w-5 shrink-0 text-right text-[9.5px] text-tinta4">
          {v ? `${v.puesto}°` : ""}
        </span>
      ...
      <div className="num mt-1.5 flex items-baseline justify-between gap-2 text-[10px] text-tinta2">
        <span className="shrink-0">
          <span className="text-local">{Math.round(p.prob.local)}</span>
          <span className="text-tinta4"> · </span>
          <span className="text-tinta2">{Math.round(p.prob.empate)}</span>
          <span className="text-tinta4"> · </span>
          <span className="text-baja">{Math.round(p.prob.visita)}</span>
          {esParejo(p.prob) && (
            <span className="ml-2 text-[9px] uppercase tracking-[0.08em] text-tinta3">
              parejo
            </span>
          )}
        </span>
        <span className="truncate text-right text-[9.5px] text-tinta3">
          xG {p.goles_esperados.local.toFixed(2)}–
          {p.goles_esperados.visita.toFixed(2)}
          <span className="hidden text-tinta4 sm:inline"> | </span>
          <span className="hidden sm:inline">
            {top.marcador} {top.prob.toFixed(0)}%
          </span>
          <span className="hidden text-tinta4 md:inline"> | </span>
